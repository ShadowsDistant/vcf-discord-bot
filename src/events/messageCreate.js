...
async function handleCountingMessage(message) {
  ...
  if (result === null) {
    // Not a number/math expression — react with ❌ and notify
    await message.react('❌').catch(() => null);
    await message.channel.send(
      `❌ ${message.author}, only numbers or math expressions are allowed in the counting channel! The count stays at **${state.count}**.`,
    ).catch(() => null);
    return true;
  }
