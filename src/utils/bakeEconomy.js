...
const ITEM_EMOJI_ALIASES = {
  ...
  caramel_macaron: ['Macaroons'],
  galaxy_stroopwafel: ['Stroopwafels'],
  ember_biscotti: ['Biscotti'],
  clockwork_madeleine: ['Madeleines'],
  aurora_graham_cracker: ['Graham_crackers'],
  mythic_peanut_butter_cookie: ['Peanut_butter_cookies'],
  stellar_shortbread: ['Shortbread_biscuits'],
  celestial_sugar_cookie: ['Sugar_cookies'],
  ascended_butter_cookie: ['Butter_cookies'],
  redacted_oatmeal_cookie: ['Oatmeal_raisin_cookies'],
};
...
const BUILDINGS = [
  ...
  { id: 'you', name: 'You', baseCost: 540000000000000000000000000, baseCps: 510000000000000, description: 'The final frontier: monetizing your own existence for cookies.' },
];
...
const UPGRADES = [
  ...
  { id: 'deep_drills', name: 'Deep Drills', category: 'building', cost: 120000, effect: '2x Mine CPS', buildingId: 'mine', multiplier: 2, unlockedWhen: (u) => (u.buildings.mine ?? 0) >= 1 },
  { id: 'assembly_lines', name: 'Assembly Lines', category: 'building', cost: 1300000, effect: '2x Factory CPS', buildingId: 'factory', multiplier: 2, unlockedWhen: (u) => (u.buildings.factory ?? 0) >= 1 },
  { id: 'vault_interest', name: 'Vault Interest', category: 'building', cost: 14000000, effect: '2x Bank CPS', buildingId: 'bank', multiplier: 2, unlockedWhen: (u) => (u.buildings.bank ?? 0) >= 1 },
  { id: 'sacred_rituals', name: 'Sacred Rituals', category: 'building', cost: 200000000, effect: '2x Temple CPS', buildingId: 'temple', multiplier: 2, unlockedWhen: (u) => (u.buildings.temple ?? 0) >= 1 },
  { id: 'arcane_scaffolds', name: 'Arcane Scaffolds', category: 'building', cost: 3300000000, effect: '2x Wizard Tower CPS', buildingId: 'wizardTower', multiplier: 2, unlockedWhen: (u) => (u.buildings.wizardTower ?? 0) >= 1 },
  { id: 'warp_logistics', name: 'Warp Logistics', category: 'building', cost: 51000000000, effect: '2x Shipment CPS', buildingId: 'shipment', multiplier: 2, unlockedWhen: (u) => (u.buildings.shipment ?? 0) >= 1 },
  { id: 'philosophers_starter', name: "Philosopher's Starter", category: 'building', cost: 750000000000, effect: '2x Alchemy Lab CPS', buildingId: 'alchemyLab', multiplier: 2, unlockedWhen: (u) => (u.buildings.alchemyLab ?? 0) >= 1 },
  { id: 'stabilized_portals', name: 'Stabilized Portals', category: 'building', cost: 10000000000000, effect: '2x Portal CPS', buildingId: 'portal', multiplier: 2, unlockedWhen: (u) => (u.buildings.portal ?? 0) >= 1 },
  { id: 'chrono_oven_gloves', name: 'Chrono Oven Gloves', category: 'building', cost: 140000000000000, effect: '2x Time Machine CPS', buildingId: 'timeMachine', multiplier: 2, unlockedWhen: (u) => (u.buildings.timeMachine ?? 0) >= 1 },
  { id: 'darkmatter_nozzles', name: 'Darkmatter Nozzles', category: 'building', cost: 1700000000000000, effect: '2x Antimatter Condenser CPS', buildingId: 'antimatterCondenser', multiplier: 2, unlockedWhen: (u) => (u.buildings.antimatterCondenser ?? 0) >= 1 },
  { id: 'spectrum_lenses', name: 'Spectrum Lenses', category: 'building', cost: 21000000000000000, effect: '2x Prism CPS', buildingId: 'prism', multiplier: 2, unlockedWhen: (u) => (u.buildings.prism ?? 0) >= 1 },
  { id: 'loaded_dice', name: 'Loaded Dice', category: 'building', cost: 260000000000000000, effect: '2x Chancemaker CPS', buildingId: 'chancemaker', multiplier: 2, unlockedWhen: (u) => (u.buildings.chancemaker ?? 0) >= 1 },
  { id: 'recursive_templates', name: 'Recursive Templates', category: 'building', cost: 3100000000000000000, effect: '2x Fractal Engine CPS', buildingId: 'fractalEngine', multiplier: 2, unlockedWhen: (u) => (u.buildings.fractalEngine ?? 0) >= 1 },
  { id: 'console_overclock', name: 'Console Overclock', category: 'building', cost: 710000000000000000000, effect: '2x Javascript Console CPS', buildingId: 'javascriptConsole', multiplier: 2, unlockedWhen: (u) => (u.buildings.javascriptConsole ?? 0) >= 1 },
  { id: 'idle_extractors', name: 'Idle Extractors', category: 'building', cost: 120000000000000000000000, effect: '2x Idleverse CPS', buildingId: 'idleverse', multiplier: 2, unlockedWhen: (u) => (u.buildings.idleverse ?? 0) >= 1 },
  { id: 'neural_fermentation', name: 'Neural Fermentation', category: 'building', cost: 19000000000000000000000000, effect: '2x Cortex Baker CPS', buildingId: 'cortexBaker', multiplier: 2, unlockedWhen: (u) => (u.buildings.cortexBaker ?? 0) >= 1 },
  { id: 'existential_crumble', name: 'Existential Crumble', category: 'building', cost: 5400000000000000000000000000, effect: '2x You CPS', buildingId: 'you', multiplier: 2, unlockedWhen: (u) => (u.buildings.you ?? 0) >= 1 },
];
...
const ACHIEVEMENTS = [
  ...
  { id: 'discover_50', name: 'Museum Curator', desc: 'Discover 25 unique items.', check: (u) => u.uniqueItemsDiscovered.length >= 25 },
...
const ITEMS = [
  ...
  buildItem('Biscotti', 'rare', 12, 'Coffee-fueled precision crunch.'),
  buildItem('Madeleine', 'rare', 13, 'Small shell-shaped nostalgia with prestige.'),
  buildItem('White Chocolate Macadamia', 'rare', 14, 'Fancy cookie with expensive opinions.'),
  buildItem('Caramel Macaron', 'epic', 45, 'Golden caramel core in a polished shell.'),
  buildItem('Galaxy Stroopwafel', 'epic', 52, 'A syrup spiral pulled from a nebula.'),
  buildItem('Ember Biscotti', 'legendary', 120, 'Smolders with concentrated coffee magic.'),
  buildItem('Clockwork Madeleine', 'legendary', 140, 'Ticks in perfect buttery rhythm.'),
  buildItem('Aurora Graham Cracker', 'mythic', 480, 'Polar lights trapped in honey grain.'),
  buildItem('Mythic Peanut Butter Cookie', 'mythic', 620, 'Ancestral recipe, divine nut ratio.'),
  buildItem('Stellar Shortbread', 'celestial', 2400, 'Crumb structure reinforced by starlight.'),
  buildItem('Celestial Sugar Cookie', 'celestial', 3200, 'Cut from pure luminous sweetness.'),
  buildItem('Ascended Butter Cookie', 'celestial', 3800, 'Layered with impossible buttery gravity.'),
  buildItem('[REDACTED] Oatmeal Cookie', 'secret', 10000, "You probably shouldn't know this recipe."),
];
